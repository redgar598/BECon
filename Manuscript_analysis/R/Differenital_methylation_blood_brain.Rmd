...
load(file=paste("~/Documents/Blood_Brain/DMA_results/Results_",tissueA,tissueB,".RData", sep=""))
...
#Truemethyl hrydroxy sites bock paper
```{r}
Hydroxy_bock<-read.csv("~/Documents/Side//Hydroxy_Methyl/Bock_hydroxy.csv")


load("Initial_BLD_BR10_hits.RData")
volcano<-data.frame(Adjusted_Pvalue=Multi_test_corr_relaxed, Delta_Beta=delbeta)
...
sta_delbeta<-delbeta[which(Multi_test_corr_relaxed<=0.001)] 
sta_delbeta<-sta_delbeta[abs(sta_delbeta)>=0.1] ## 120390
length(sta_delbeta[which(sta_delbeta>=dB)]) ##  66064
length(sta_delbeta[which(sta_delbeta<=(-dB))]) ## 54326
...
##hydroxy #high condfidence hydroxy (objects from hydroxy script)
hmc<-volcano[which(rownames(volcano)%in%Hydroxy_bock$probeID),]
high_hmc<-hmc[which(rownames(hmc)%in%high_confidence_hydroxy),]
high_hmc$hmc<-"High Confidence Hydroxy"
low_hmc<-hmc[which(!(rownames(hmc)%in%high_confidence_hydroxy)),]#59785
low_hmc$hmc<-"Low Confidence Hydroxy"
mc<-volcano[which(!(rownames(volcano)%in%Hydroxy_bock$probeID)),]#381413
mc$hmc<-"5hmc Not Detected"

volcano<-rbind(high_hmc,low_hmc,mc)
...
#omg
library(scales)
ggplot(volcano, aes(Delta_Beta, -log10(Adjusted_Pvalue), color=Interesting_CpG3))+geom_point(shape=19, size=1)+theme_bw()+scale_y_log10()+scale_color_manual(values=c(muted("red", l=80, c=30),"red",muted("blue", l=70, c=40),"blue","grey"),name="CpG Change Between \n Blood and Brain")+geom_vline(xintercept=c(-dB,dB), color="grey60")+geom_hline(yintercept=-log10(Pv), color="grey60")+ylab("Multiple Test Corrected P Value (-log10)")+xlab("Delta Beta")+xlim(-1, 1)+theme(axis.text = element_text(size =14, color="black"),
          axis.title = element_text(size =20),
          legend.text = element_text(size =14),
          legend.title = element_text(size =20))+ 
  guides(color = guide_legend(override.aes = list(size = 4)))+facet_wrap(~hmc)
```

```{r}
x<-tapply(rownames(volcano), list(volcano$Interesting_CpG3, volcano$hmc), length)
y<-colSums(x)
a<-rowSums(x)

apply(x,1, function(z) z/y)
apply(x,2, function(z) z/a)
